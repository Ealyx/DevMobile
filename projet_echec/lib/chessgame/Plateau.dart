import 'package:ChessArena/chessgame/pieces/Cavalier.dart';
import 'package:ChessArena/chessgame/pieces/Dame.dart';
import 'package:ChessArena/chessgame/pieces/Fou.dart';
import 'package:ChessArena/chessgame/pieces/Piece.dart';
import 'package:ChessArena/chessgame/pieces/Tour.dart';
import 'package:ChessArena/chessgame/pieces/Roi.dart';
import 'package:ChessArena/chessgame/pieces/Pion.dart';
import 'package:ChessArena/chessgame/log.dart';
import 'package:flutter/material.dart';
import 'package:ChessArena/timer.dart';
...
class Plateau {
  ...
  void chargerPartie(Log database) {
    Reinitialisation();
    print("Chargement de la partie");
    database.getSaves().then((saves) {
      if (saves.isNotEmpty) {
        for (var save in saves) {
          print("Chargement de la partie : $save");
          plateau[save.y][save.x]
              ?.deplacement(save.newX, save.newY, plateau as Plateau, database);
          timerB.setMinutes(0);
          timerB.setSeconds(0);
          timerB.addSeconds(save.timerB);
          timerN.setMinutes(0);
          timerN.setSeconds(0);
          timerN.addSeconds(save.timerN);
          tourActuel = save.turn;
          if (save.pieceCapturee == 1) {
            Piece? pieceCapturee;
            String couleurP = save.turn == "Blanc" ? "Noir" : "Blanc";
            switch (save.pieceCaptureePiece) {
              case "Dame":
                pieceCapturee = Dame(couleurP, 0, 0,
                    "$path${couleurP == "Blanc" ? "white" : "black"}-queen.png");
                break;
              case "Pion":
                pieceCapturee = Pion(couleurP, 0, 0,
                    "$path${couleurP == "Blanc" ? "white" : "black"}-pawn.png");
                break;
              case "Cavalier":
                pieceCapturee = Cavalier(couleurP, 0, 0,
                    "$path${couleurP == "Blanc" ? "white" : "black"}-knight.png");
                break;
              case "Fou":
                pieceCapturee = Fou(couleurP, 0, 0,
                    "$path${couleurP == "Blanc" ? "white" : "black"}-bishop.png");
                break;
              case "Tour":
                pieceCapturee = Tour(couleurP, 0, 0,
                    "$path${couleurP == "Blanc" ? "white" : "black"}-rook.png");
                break;
              case "Roi":
                pieceCapturee = Roi(couleurP, 0, 0,
                    "$path${couleurP == "Blanc" ? "white" : "black"}-king.png");
                break;
              default:
                pieceCapturee = null;
            }
            if (pieceCapturee != null) {
              piecesMortes.add(pieceCapturee);
            }
          }
        }
        if (tourActuel == "Blanc") {
